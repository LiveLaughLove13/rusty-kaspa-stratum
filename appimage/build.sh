...
# Layout mirrors kaspanet/rusty-kaspa `bridge/appimage/`, but paths use this repo’s root `static/`.
#
# Usage: from repo root, after:
#   cargo build --release --bin stratum-bridge --target x86_64-unknown-linux-musl
# run:
#   bash appimage/build.sh [version-label]
set -euo pipefail
...
mkdir -p "$ICON_DIR"
SVG="${ROOT}/static/assets/kaspa.svg"
...
if [[ -f "$SVG" ]] && command -v rsvg-convert >/dev/null 2>&1; then
  ...
  echo "error: missing static/assets/kaspa.svg and bundled stratum-bridge.png; cannot produce app icon." >&2
  exit 1
fi
