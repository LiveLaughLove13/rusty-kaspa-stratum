...
mod tests {
    ...
    fn diff_to_target_higher_diff_means_smaller_target() {
        ...
        assert!(
            t_high < t1,
            "larger stratum difficulty should yield smaller target"
        );
    }
    ...
    fn diff_to_target_non_positive_uses_max_target() {
        ...
        assert!(
            t0 > t_two,
            "invalid/zero diff should be strictly easier than diff=2.0"
        );
    }
    ...
    fn test_share_validation_comparison() {
        ...
        if pow_value.to_bytes_be()[0] < target_8192.to_bytes_be()[0] {
            ...
            println!(
                "WARNING: First byte comparison suggests invalid share, but numeric comparison may differ"
            );
        }
    }
}
