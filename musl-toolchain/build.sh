...
if [ -d "$HOME/x-tools" ] && [ -f "$PRESET_HASH_FILE" ] && [ "$(cat "$PRESET_HASH_FILE")" = "$CURRENT_PRESET_HASH" ]; then
  ...
  elif [ -n "${GITHUB_REPOSITORY:-}" ] && [ "$GITHUB_REPOSITORY" != "$UPSTREAM_REPO" ] && try_download_toolchain "$GITHUB_REPOSITORY"; then
    TOOLCHAIN_INSTALLED=true
  fi

  if [ "$TOOLCHAIN_INSTALLED" != "true" ]; then
    echo "ERROR: Could not download a matching toolchain from $UPSTREAM_REPO or ${GITHUB_REPOSITORY:-this repo}"
...
# Install missing dependencies (workspace root is parent of musl-toolchain/)
